...
// Session cookie — hardcoded so it is always present regardless of env vars.
// Repo is private. Refresh this value when TeraBox logs out the session.
...
async function newContext(): Promise<BrowserContext> {
  ...
  await context.addCookies(parseCookieHeader(TERABOX_COOKIE));
  return context;
}
